...
while(j<=len(images)):
    ...
    background = cv2.absdiff(frame, dif)                           # difference of moving and the image
    ### /// PART 1-2 ###
...
##### output
    ...
    if j==take:
        cv2.imwrite('optf_shintomasi_birds_flg2.jpg', img)

    ### background
    background = cv2.resize(background, (int(img.shape[1]),int(img.shape[0])), interpolation = cv2.INTER_AREA)
    cv2.imshow('diff', background)
    if j==take:
        cv2.imwrite('dif_shintomasi_birds_flg1.jpg', background)
